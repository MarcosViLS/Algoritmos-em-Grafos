...
//definindo uma aresta com elementos: custo, vértice de partida, e vértice de chegada
struct Aresta {
	int custo;
	char vPartida;
	char vDestino;
...
//pq --> codinome para priority_queue
	...
	visitados['A'] = false; //"inicializando vértice A como não visitado"...
	visitados['B'] = false;
	visitados['C'] = false;
	visitados['D'] = false;
	visitados['E'] = false;
...
//declarando lista de adjacência (representação do grafo)
    map<char, vector<Aresta>> adj;

    //criando grafo
    adj['A'].push_back({2,'A','B'});  //"vértice A tem aresta adjacente de custo 2 ligando a B"...
    adj['A'].push_back({3,'A','C'});
    
    adj['B'].push_back({2,'B','A'});
    adj['B'].push_back({1,'B','C'});
    adj['B'].push_back({4,'B','D'});
    
    adj['C'].push_back({3,'C','A'});
    adj['C'].push_back({1,'C','B'});
    adj['C'].push_back({5,'C','D'});
    adj['C'].push_back({6,'C','E'});
    
    adj['D'].push_back({4,'D','B'});
    adj['D'].push_back({5,'D','C'});
    adj['D'].push_back({2,'D','E'});
    adj['D'].push_back({3,'D','F'});
    
    adj['E'].push_back({6,'E','C'});
    adj['E'].push_back({2,'E','D'});
    adj['E'].push_back({4,'E','F'});
    
    adj['F'].push_back({3,'F','D'});
    adj['F'].push_back({4,'F','E'}); 
...
cout<<"Vértices visitados: "<<inicio<<endl;
cout<<endl;
...
for (auto &v : visitados) {
    if (v.second) { // só imprime se já foi visitado
        cout << v.first << " ";
    }
}
cout << endl;
...
cout << "Custo total: " << custoTotal << endl; //exibir custo total
}
...
//4.A ordenação por custo feita dentro da lógica de priority_queue
